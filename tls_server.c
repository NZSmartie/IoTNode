...
void tls_server_task(void *pvParameters)
{
    ...
    while(1) {
         ...
         */
        ret = mbedtls_net_bind(&server_ctx, NULL, PORT, MBEDTLS_NET_PROTO_UDP);
        if(ret != 0)
        {
        ...
        ret=mbedtls_net_accept(&server_ctx, &client_ctx, client_ip, sizeof(client_ip) , &cliip_len);
        if( ret != 0 ){
